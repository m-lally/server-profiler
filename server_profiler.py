...
class ServerProfiler:
    ...
    def __init__(self, ssh_connector: SSHConnector, console: Optional[Console] = None,
                 verbose: bool = False):
        """
    ...
    def profile_all(self) -> Dict[str, Any]:
        ...
        """
        self.console.print("[bold cyan]⟐ Profiling server...[/bold cyan]")

        steps = [
            ("OS information", self.get_os_info, "os_info"),
            ("Hardware information", self.get_hardware_info, "hardware"),
            ("Network configuration", self.get_network_info, "network"),
            ("Installed packages", self.get_installed_packages, "packages"),
            ("Running services", self.get_services, "services"),
            ("Firewall rules", self.get_firewall_rules, "firewall"),
            ("Storage configuration", self.get_storage_info, "storage"),
            ("User accounts", self.get_users, "users"),
            ("Cron jobs", self.get_cron_jobs, "cron_jobs"),
            ("Software detection", self.detect_installed_software, "software"),
        ]
        ...
        with Progress(
            ...
            for label, method, key in steps:
                task = progress.add_task(f"[cyan]{label}...", total=None)
                self.profile[key] = method()
    ...
    def get_cron_jobs(self) -> Dict[str, List[str]]:
        ...
        for cron_dir in ['/etc/cron.d', '/etc/cron.daily', '/etc/cron.hourly',
            ...
            stdout, _, exit_code = self.ssh.execute_command(
                f"ls -1 {cron_dir} 2>/dev/null"
            )
            if exit_code == 0 and stdout:
                cron_jobs['system'].extend(
                    [f"{cron_dir}/{f.strip()}" for f in stdout.split('\n') if f.strip()]
                )
    ...
    def detect_installed_software(self) -> Dict[str, Any]:
        ...
        for lang, cmd in [('python', 'python3 --version'),
                          ('node', 'node --version'),
                          ('ruby', 'ruby --version'),
                          ('php', 'php --version'),
                          ('go', 'go version'),
        ...
        if exit_code == 0:
            ...
            if stdout:
                software['docker']['running_containers'] = [
                    c.strip() for c in stdout.split('\n') if c.strip()
                ]
    ...
    def save_profile(self, output_file: str = 'profile.json'):
        ...
        self.console.print(f"\n[bold green]✓[/bold green] Profile saved to [cyan]{output_file}[/cyan]")
